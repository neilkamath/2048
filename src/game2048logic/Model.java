...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int x = 0; x < this.size(); x++) {
            for (int y = 0; y < this.size(); y++) {
                boolean horizontalCheck = x + 1 < this.size() && tile(x, y).value() == tile(x + 1, y).value();
                boolean verticalCheck = y + 1 < this.size() && tile(x, y).value() == tile(x, y + 1).value();
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        for (int i = y + 1; i < board.size(); i++) {
            ...
            if (board.tile(x, i) != null && board.tile(x, i).value() == myValue && !board.tile(x, i).wasMerged()) {
                targetY++;
            }
        }
        if (targetY != y) {
        ...
        if (myValue != board.tile(x, targetY).value()) {
            score += 2 * myValue;
        }
    }
